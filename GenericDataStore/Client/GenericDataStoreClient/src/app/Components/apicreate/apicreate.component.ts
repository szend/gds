...
@Component({
  ...
  templateUrl: './apicreate.component.html',
  styleUrl: './apicreate.component.css'
})
  ...
  Save() {
    ...
    if(this.config.data.createmode == true){
      this.apiService.Connect(this.database).subscribe((data: any) => {
        this.loading = false;
        this.messageService.add({severity:'success', summary: 'Success', detail: 'API connection created successfully'});
      }, (error: any) => {
        this.loading = false;
        this.messageService.add({severity:'error', summary: 'Error', detail: 'API connection failed'});
      });
    }
    else if(this.config.data.createmode == false){
      this.apiService.EditConnectionAPI(this.database).subscribe((data: any) => {
        this.loading = false;
        this.messageService.add({severity:'success', summary: 'Success', detail: 'API connection updated successfully'});
      }, (error: any) => {
        this.loading = false;
        this.messageService.add({severity:'error', summary: 'Error', detail: 'API connection update failed'});
      });
    }
  }
