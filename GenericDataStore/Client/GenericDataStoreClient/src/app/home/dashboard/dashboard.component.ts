...
export class DashboardComponent  implements OnInit {
  ...
  Refresh(all : boolean = true){
    ...
    this.apiService.IsDashboardPublic().subscribe(x => {
      this.publicdashboard = x;
    });
  ...
  SaveDashboardPublic(){
    if(!this.publicdashboard){
      this.apiService.SetDashoardNonPublic().subscribe(x => {
        this.publicdashboard = false;
      });
    }
    else{
      this.apiService.SetDashoardPublic().subscribe(x => {
